...
# v0.7.0 identity-based message signatures (Ed25519 hot key)
try:
    from identity import get_or_create_hot_key, sign_message_dict
except Exception:
    get_or_create_hot_key = None
    sign_message_dict = None

# ============ CONFIG ============
def get_secret() -> str:
    """Load shared secret from encrypted store (legacy)."""
    try:
...
def get_peers():
    ...
    NOTE: In v0.7.0 'zero shared secret' mode, peers do NOT need a secret.
    Secret-based Authorization is kept only as an optional legacy fallback.
    """
    return {
        "neo": {"url": "http://89.168.70.9:8080"},
        "zen": {"url": "http://35.158.138.126:8080"},
    }

PEERS = None  # Lazy loaded
DEFAULT_PEER = "neo"
SENDER_NAME = "Zen"
...
def send_message(peer_name: str, message: str, wake: bool = True, trace_id: str = None, use_secret: bool = False) -> dict:
    """
    ...
    # v0.7.0: attach identity + signature (required when REQUIRE_SIGNATURE=True on server)
    if get_or_create_hot_key and sign_message_dict:
        hk = get_or_create_hot_key()
        payload["identity"] = {
            "hot_pub_b64": base64.b64encode(hk.pub_raw32()).decode("ascii"),
        }
        payload["sig"] = sign_message_dict(hk, payload)
    else:
        # Keep backward-compat, but servers with REQUIRE_SIGNATURE=True will reject.
        pass
...
def main():
    ...
    parser.add_argument("--use-secret", action="store_true", help="(Legacy) Send Bearer shared secret in Authorization header")
